...
impl EvolutionEngine {
    ...
    pub fn evolve(&mut self, manager: &mut StrategyWalletManager) -> EvolutionReport {
        ...
        // 1. Rank by Sharpe — collect as owned Strings to avoid lifetime conflicts
        let ranked: Vec<(String, f64)> = manager.rank_by_sharpe(self.min_trades_for_ranking)
            .into_iter()
            .map(|(name, sharpe)| (name.to_string(), sharpe))
            .collect();
        let ranked_count = ranked.len();

        // Save best for report before any mutation
        ...
        if ranked_count >= 3 {
            ...
            for name in &to_kill {
                ...
                if let Some(slot) = manager.get_mut(name) {
                    ...
                    info!("EVOLUTION KILL: {}", name);
                }
            }

            // 3. Spawn from top 10% — collect all data needed before mutating manager
