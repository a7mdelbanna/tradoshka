...
| 2026-03-28 | **Crypto ATR 40x too small → inverted R:R** | Synthetic warmup used `price * 0.001` (0.1%) walk_size, producing ATR of 0.05% instead of realistic 1.5%. SL/TP were within bid-ask noise (SL=0.10%, TP=0.05%). R:R was 0.30-0.75 instead of target 2.0. Win rate: 3-8%. | Walk_size must match asset's real volatility. For crypto: `price * 0.015` (1.5%). Always verify actual SL/TP distances from trade data, not just the params |
| 2026-03-28 | Strategy R:R params ignored | `take_profit_rr` param existed in strategy definitions (1.5-4.4 range) but ResearchConfig hardcoded `min_rr_ratio: 2.0`. All strategies had identical R:R regardless of params | Always wire strategy params to execution config. Params that aren't used are lies |
